...
class FuzzingBLESUL(BLESUL):
    ...
    def react_to_request(self, response):
        ...
        while response is not None:
            if(self.check_rsp(response)):
                if LL_SLAVE_FEATURE_REQ().summary() in response:
                    response = response.replace(LL_SLAVE_FEATURE_REQ().summary(),"")
                    response = self.feature_request()
                elif LL_FEATURE_RSP().summary() in response:
                    response = response.replace(LL_LENGTH_REQ().summary(),"")
                    response = self.length_request()
                elif LL_LENGTH_RSP().summary() in response:
                    response = response.replace(ATT_Exchange_MTU_Request().summary(),"")
                    response = self.mtu_request()
                elif ATT_Exchange_MTU_Response().summary() in response:
